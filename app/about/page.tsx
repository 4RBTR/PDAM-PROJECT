...
export default function AboutPage() {
    ...
    return (
        ...
        <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900/50 selection:text-indigo-700 dark:selection:text-indigo-300 flex flex-col relative transition-colors duration-300">
            ...
            <div className="fixed top-0 left-0 right-0 z-100 flex justify-center p-4 md:p-6 pointer-events-none">
                <nav className={`
                    ...
                    ${isMobileMenuOpen || scrolled
                        ...
                        : 'bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border-white/40 dark:border-slate-800/40 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:bg-white/80 dark:hover:bg-slate-900/80 hover:border-white/60 dark:hover:border-slate-700'}
                `}>
                    ...
                    <div className="px-5 md:px-8 h-14 md:h-17 flex justify-between items-center relative z-10">
                        ...
                        <Link href="/" className="flex items-center gap-3 group/logo">
                            ...
                            <div className="hidden sm:flex flex-col gap-0">
                                <span className="font-bold text-lg tracking-tight text-slate-900 dark:text-white leading-tight">PDAM<span className="text-indigo-600 dark:text-indigo-400">.</span></span>
                        ...
                        <div className="hidden md:flex items-center gap-2">
                            {['About', 'Services', 'Contact'].map((item) => (
                                <Link key={item} href={`/${item.toLowerCase()}`} className={`px-4 py-2 text-[13px] font-bold rounded-full transition-all duration-300 relative group/item ${item === 'About' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-300'}`}>
                                    <span className="relative z-10">{item}</span>
                                    <span className="absolute inset-0 bg-slate-900/5 dark:bg-white/5 rounded-full scale-50 opacity-0 group-hover/item:scale-100 group-hover/item:opacity-100 transition-all duration-300"></span>
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-3 md:gap-5">
                            <Link href="/login" className="hidden md:block text-[13px] font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Masuk</Link>
                            <Link href="/register" className="px-6 py-2.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-full text-[13px] font-bold shadow-[0_10px_20px_-5px_rgba(15,23,42,0.3)] dark:shadow-[0_10px_20px_-5px_rgba(255,255,255,0.1)] hover:shadow-indigo-500/40 hover:bg-indigo-600 dark:hover:bg-indigo-500 dark:hover:text-white hover:-translate-y-0.5 active:scale-95 transition-all duration-300">Get Started</Link>
                            <button className="md:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 rounded-xl bg-slate-100/50 dark:bg-slate-800/50" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                                <div className={`h-0.5 bg-slate-900 dark:bg-white rounded-full transition-all duration-300 ${isMobileMenuOpen ? 'w-5 rotate-45 translate-y-1' : 'w-5'}`}></div>
                                <div className={`h-0.5 bg-slate-900 dark:bg-white rounded-full transition-all duration-300 ${isMobileMenuOpen ? 'w-5 -rotate-45 -translate-y-1' : 'w-3 self-end mr-2.5'}`}></div>
                            </button>
                        </div>
                    </div>

                    {/* Mobile Menu */}
                    <div className={`md:hidden overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isMobileMenuOpen ? 'max-h-100 border-t border-slate-100 dark:border-slate-800 opacity-100 scale-100' : 'max-h-0 opacity-0 scale-95'}`}>
                        <div className="p-6 space-y-1">
                            {['About', 'Services', 'Contact'].map((item) => (
                                <Link key={item} href={`/${item.toLowerCase()}`} className="flex items-center p-3 text-base font-bold text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all">{item}</Link>
                            ))}
                            <div className="pt-4 mt-2 border-t border-slate-50 dark:border-slate-800/50">
                                <Link href="/login" className="block w-full py-4 text-center font-bold text-slate-900 dark:text-white rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition">Masuk ke Akun</Link>
                            </div>
                        </div>
                    </div>
                </nav>
            ...
            <main className="grow pt-32 pb-24 md:pt-44 container mx-auto px-6 relative z-10 max-w-5xl">
                ...
                <div className="text-center mb-20 md:mb-24">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800/50 rounded-full mb-6">
                        <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                        <span className="text-[11px] font-bold tracking-widest text-indigo-600 dark:text-indigo-400 uppercase">Our Story</span>
                    </div>
                    <h1 className="text-5xl md:text-7xl font-black mb-6 text-slate-900 dark:text-white tracking-tighter leading-[0.95]">
                        Revolusi Air <br className="hidden md:block" />
                        <span className="text-transparent bg-clip-text bg-linear-to-r from-slate-900 via-indigo-600 to-slate-900 dark:from-white dark:via-indigo-400 dark:to-white">Digital.</span>
                    </h1>
                ...
                <div className="space-y-8 md:space-y-12">
                    {/* Introduction Card */}
                    <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-4xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.05)] dark:shadow-none border border-slate-100 dark:border-slate-800 relative overflow-hidden group transition-colors">
                        <div className="absolute top-0 right-0 w-75 h-75 bg-linear-to-bl from-indigo-50/80 dark:from-indigo-900/20 to-transparent rounded-bl-[100px] transition-transform duration-700 group-hover:scale-110"></div>
                        <div className="relative z-10 md:w-3/4">
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Siapa Kami?</h2>
                            <p className="text-slate-600 dark:text-slate-300 text-lg leading-loose">
                                <strong className="text-indigo-600 dark:text-indigo-400">PDAM Pintar AI</strong> bukan sekadar aplikasi pembayaran. Kami adalah ekosistem teknologi yang menjembatani kesenjangan antara penyedia layanan air dan masyarakat.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                        <div className="bg-slate-900 p-10 rounded-4xl text-white flex flex-col justify-between group hover:shadow-2xl hover:shadow-indigo-500/20 transition-all duration-300 border border-transparent dark:border-slate-800">
                            <div>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-10 rounded-4xl border border-slate-200 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-800 hover:shadow-xl transition-all duration-300">
                            <div className="w-12 h-12 bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 rounded-2xl flex items-center justify-center text-2xl mb-6">🚀</div>
                            <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-6 tracking-tight">Misi Kami</h3>
                            <ul className="space-y-4">
                                {["Transparansi data real-time.", "Penghapusan antrian fisik.", "Respon AI < 5 menit.", "Edukasi konservasi air."].map((item, i) => (
                                    <li key={i} className="flex items-center gap-3 text-slate-600 dark:text-slate-300 font-medium">
                                        <span className="w-1.5 h-1.5 bg-indigo-600 dark:bg-indigo-400 rounded-full"></span>{item}
                                    </li>
                                ))}
                            </ul>
                        </div>
