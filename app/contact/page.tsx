...
export default function ContactPage() {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900/50 selection:text-indigo-700 dark:selection:text-indigo-300 flex flex-col relative overflow-x-hidden transition-colors duration-300">
                ...
                `}>
                    ...
                    <div className="px-5 md:px-8 h-14 md:h-17 flex justify-between items-center relative z-10">
                        ...
                        <div className="hidden md:flex items-center gap-2">
                            {['About', 'Services', 'Contact'].map((item) => (
                                <Link
                                    ...
                                    className={`px-4 py-2 text-[13px] font-bold rounded-full transition-all duration-300 relative group/item ${item === 'Contact' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-300'}`}
                                >
                        ...
                        <div className="flex items-center gap-3 md:gap-5">
                            ...
                            {/* Mobile Toggle */}
                            <button
                                className="md:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 rounded-xl bg-slate-100/50 dark:bg-slate-800/50"
                    ...
                    <div className={`
                        ...
                        ${isMobileMenuOpen ? 'max-h-100 border-t border-slate-100 dark:border-slate-800 opacity-100 scale-100' : 'max-h-0 opacity-0 scale-95'}
                    `}>
                        <div className="p-6 space-y-1">
                            {['About', 'Services', 'Contact'].map((item) => (
                                <Link key={item} href={`/${item.toLowerCase()}`} className="flex items-center p-3 text-base font-bold text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all">
                                    {item}
                                </Link>
                            ))}
                ...
                <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-start">
                    ...
                    <div className="space-y-10 md:sticky md:top-36">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800/50 rounded-full mb-6">
                                ...
                                <span className="text-[11px] font-bold tracking-widest text-indigo-600 dark:text-indigo-400 uppercase">Support 24/7</span>
                            </div>
                            <h1 className="text-4xl md:text-6xl font-black mb-6 text-slate-900 dark:text-white tracking-tighter leading-tight">
                                Hubungi <br />
                                <span className="text-transparent bg-clip-text bg-linear-to-r from-indigo-600 via-indigo-600 to-sky-500 dark:from-indigo-400 dark:to-sky-400">Tim Kami.</span>
                            </h1>
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={idx} className="flex items-center gap-5 p-5 rounded-[20px] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_5px_20px_-5px_rgba(0,0,0,0.03)] dark:shadow-none hover:shadow-lg dark:hover:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] hover:border-indigo-100 dark:hover:border-indigo-800 hover:-translate-y-0.5 transition-all duration-300 group">
                                    <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-2xl group-hover:scale-110 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/30 transition-all duration-300">
                                        {item.icon}
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-slate-900 dark:text-slate-100 text-lg group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{item.title}</h4>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-4xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] dark:shadow-none border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-colors">
                        ...
                        <div className="relative z-10 mb-8">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">Kami akan membalas pesan Anda sesegera mungkin.</p>
                        </div>

                        <form className="space-y-5 relative z-10" onSubmit={handleSubmit}>
                            <div className="space-y-1.5">
                                <label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 ml-1">Nama Lengkap</label>
                                <input
                                    required
                                    type="text"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 ml-1">Pesan / Keluhan</label>
                                <textarea
                                    required
                                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all h-32 placeholder:text-slate-400 dark:placeholder:text-slate-500 resize-none font-medium"
