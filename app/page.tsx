...
export default function Home() {
  ...
  return (
    // WRAPPER UTAMA
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-indigo-500 selection:text-white overflow-x-hidden flex flex-col relative antialiased transition-colors duration-300">
      ...
      <main className="pt-32 pb-20 md:pt-48 md:pb-32 container mx-auto px-4 md:px-6 relative z-10 flex flex-col lg:flex-row items-center gap-16 lg:gap-20">
        ...
        <div className="flex-1 w-full text-center lg:text-left space-y-8">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white dark:bg-slate-900/50 border border-sky-100 dark:border-sky-900/50 text-sky-700 dark:text-sky-400 text-[11px] font-bold uppercase tracking-wider shadow-[0_2px_10px_rgba(14,165,233,0.15)] mx-auto lg:mx-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-slate-900 dark:text-white leading-[1.05] tracking-tight animate-in fade-in slide-in-from-bottom-6 duration-700 delay-100">
            Bayar Air <br className="hidden lg:block" />
            <span className="text-transparent bg-clip-text bg-linear-to-r from-sky-500 via-indigo-500 to-violet-500 dark:from-sky-400 dark:via-indigo-400 dark:to-violet-400">Tanpa Antri.</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start w-full sm:w-auto animate-in fade-in slide-in-from-bottom-6 duration-700 delay-300">
            ...
            <Link href="/login" className="px-8 py-4 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 rounded-2xl font-bold hover:bg-slate-50 dark:hover:bg-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700 transition-all shadow-sm hover:shadow-md active:scale-95 flex items-center justify-center">
              Lihat Demo
            </Link>
          </div>
              ...
              <div className="bg-slate-50 rounded-4xl overflow-hidden border border-slate-100/50">
                ...
                <div className="p-6 md:p-8 space-y-6 bg-slate-50/50 dark:bg-slate-900/50">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { i: '💧', t: 'Meteran', c: 'bg-sky-50 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/60 border-sky-100 dark:border-sky-800' },
                      { i: '📜', t: 'Riwayat', c: 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 border-indigo-100 dark:border-indigo-800' },
                      { i: '🎧', t: 'Bantuan', c: 'bg-rose-50 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/60 border-rose-100 dark:border-rose-800' },
                    ].map((m, idx) => (
                      <div key={idx} className={`bg-white dark:bg-slate-800 p-3 rounded-2xl shadow-sm border ${m.c} border-opacity-50 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all hover:-translate-y-1`}>
                        <span className="text-xl">{m.i}</span>
                        <span className="text-[10px] font-bold dark:text-slate-200">{m.t}</span>
                      </div>
                    ))}
                  </div>
      ...
      <section className="py-24 bg-white dark:bg-slate-900 relative z-10 transition-colors duration-300">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <span className="text-indigo-600 dark:text-indigo-400 font-bold tracking-wider uppercase text-xs mb-2 block">Kenapa Memilih Kami?</span>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-slate-100 mb-6 tracking-tight">Fitur Lengkap untuk <span className="underline decoration-indigo-400/30 decoration-4 underline-offset-4">Kenyamanan</span> Anda</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg">Teknologi terbaik untuk kenyamanan Anda dalam mengakses air bersih, didesain untuk kemudahan maksimal.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { t: "Real-time Monitoring", d: "Pantau penggunaan air harian dari HP Anda secara akurat tanpa perlu menunggu tagihan akhir bulan.", i: "📊", c: "bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 group-hover:bg-blue-600 group-hover:text-white" },
              { t: "Pembayaran Instan", d: "Dukung QRIS, Virtual Account Bank & E-Wallet. Konfirmasi otomatis dalam hitungan detik.", i: "💳", c: "bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-600 group-hover:text-white" },
              { t: "Layanan Pengaduan", d: "Lapor kebocoran atau gangguan air dengan tiket prioritas dan tracking status pengerjaan.", i: "🛠️", c: "bg-orange-50 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400 group-hover:bg-orange-600 group-hover:text-white" },
            ].map((f, i) => (
              <div key={i} className="group relative bg-white dark:bg-slate-800 p-8 rounded-4xl border border-slate-100 dark:border-slate-700 hover:border-transparent dark:hover:border-transparent hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] transition-all duration-300 hover:-translate-y-2 overflow-hidden">
                <div className="absolute inset-0 bg-linear-to-br from-slate-50 to-white dark:from-slate-800 dark:to-slate-700 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="relative z-10">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{f.t}</h3>
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-sm group-hover:text-slate-600 dark:group-hover:text-slate-300">{f.d}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* --- CTA BANNER --- */}
      <section className="py-16 md:py-24 bg-slate-50 dark:bg-slate-950/50 relative z-10 transition-colors">
